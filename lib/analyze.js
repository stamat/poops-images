...
import { markMainSizes } from './sizes.js'
import { dmsToDecimal, formatDms } from './utils/format.js'
import log from './utils/log.js'

export function buildEffectiveSizes(sizes, skipOriginal) {
  if (skipOriginal) return markMainSizes(sizes)

  const hasPassthrough = sizes.some(s => s.width === 0 && s.height === 0)
  if (hasPassthrough) return markMainSizes(sizes)

  return markMainSizes([{ name: '', width: 0, height: 0, crop: false }, ...sizes])
}
...
export async function analyzeImage(inputPath, config, stat) {
  ...
  // Default mode: normalize non-web formats. The notices are per-file
  // progress, so they obey verbose like every other per-file log.
  if (config.format === false) {
    if (originalExt === 'png' && !transparent) {
      outputExt = 'jpg'
      if (config.verbose) log({ tag: 'image', text: 'Opaque PNG \u2192 JPEG:', link: relativePath })
    } else if (nonWebFormats.has(normalizedExt)) {
      const label = normalizedExt.toUpperCase()
      if (transparent) {
        outputExt = 'png'
        if (config.verbose) log({ tag: 'image', text: `Transparent ${label} \u2192 PNG:`, link: relativePath })
      } else {
        outputExt = 'jpg'
        if (config.verbose) log({ tag: 'image', text: `${label} \u2192 JPEG:`, link: relativePath })
      }
    }
  }
