...
export default class Cache {
  ...
  // Every recorded output as an absolute path, minus files that are their own
  // output (in-place conversion when in == out) — those must stay discoverable
  // as sources. Discovery uses this to keep written variants from being
  // re-ingested as sources on the next run.
  foreignOutputs(inputDir) {
    const set = new Set()
    for (const [srcRel, entry] of Object.entries(this.data.entries)) {
      const src = path.resolve(inputDir, srcRel)
      for (const output of entry.outputs || []) {
        const outputPath = typeof output === 'string' ? output : output.path
        const abs = path.resolve(this.outputDir, outputPath)
        if (abs !== src) set.add(abs)
      }
    }
    return set
  }
