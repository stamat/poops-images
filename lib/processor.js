...
import { toSharpOptions, filterByUpscale, clampSizeToSource } from './sizes.js'
import { convertFormat, resolveTargetFormats } from './formats.js'
import { discoverAll, isDiscovered } from './discover.js'
...
export default class ImageProcessor {
  ...
  // With in == out, a conversion-only variant's path is its own source.
  // Writing it would recompress the original in place, and the cache would
  // hold the pre-write mtime — so every subsequent run recompresses the
  // recompression. Refuse the write, loudly, and let the sized variants
  // (whose names carry a suffix) proceed.
  _refusesOverwrite(outputPath, sourcePath, relativePath) {
    if (path.resolve(outputPath) !== path.resolve(sourcePath)) return false
    this._error({ text: 'Refusing to overwrite a source with its own output:', link: relativePath })
    return true
  }
  ...
  async processAll(options = {}) {
    ...
    // Discover all file types in parallel. Anything the cache records as an
    // output of another file is not a source — this is what stops in == out
    // setups from re-ingesting their own variants run after run.
    const discovered = await discoverAll(this.config)
    const written = this.cache.foreignOutputs(this.config.in)
    const notOwn = files => files.filter(f => !written.has(path.resolve(f)))
    const sourceFiles = notOwn(discovered.raster)
    const svgFiles = notOwn(discovered.svg)
    const gifFiles = notOwn(discovered.gif)
  ...
  async processImage(inputPath, force = false) {
    ...
    // 1. Original pipeline. A source that passes metadata can still fail at
    // decode (truncated body) or at write — contain it to this file and bail
    // before the stale cleanup below, which would otherwise read the empty
    // outputs list as "everything is stale" and delete the previous run's
    // good variants.
    try {
      await this._processVariants(inputPath, job, null, outputs, startTime)
    } catch (err) {
      this._error({ text: `Processing failed: ${err.message}`, link: relativePath })
      return
    }
  ...
  async _processVariants(input, job, preprocessor, outputs, startTime, baseDims = null) {
    ...
    if (preprocessor?.sizes) {
      // Already normalized by validatePreprocessor
      sizes = buildEffectiveSizes(preprocessor.sizes, skipOrig)
    ...
    for (const sizeDef of eligibleSizes) {
      ...
      // Crops on an undersized source scale down to the largest box that fits,
      // preserving the crop aspect ratio — never upscaled, never dropped.
      const effSize = clampSizeToSource(sizeDef, srcWidth, srcHeight)

      // Resize — flush to raw pixels; the only encode is the final one per format
      const sharpOpts = toSharpOptions(effSize)
      ...
      // The largest of each named group drops the width suffix — it's the
      // canonical "main" variant (`photo-thumb.webp`); smaller siblings keep
      // `-{width}w` so poops can assemble a srcset for the group.
      let baseName
      if (conversionOnly) {
        baseName = namePrefix
      } else if (sizeDef.name && sizeDef.main) {
        baseName = `${namePrefix}-${sizeDef.name}`
      ...
      for (const targetExt of targetFormats) {
        ...
        // input is the source path only in the original pipeline; preprocessed
        // buffers carry the preprocessor name in baseName and cannot collide
        if (typeof input === 'string' && this._refusesOverwrite(outputPath, input, relativePath)) continue

        fs.writeFileSync(outputPath, buffer)
        // `name` tags named-size variants so poops can group them into their own
        // srcset (a crop group has its own aspect ratio, separate from the widths).
        const output = { path: relPath, width: actualWidth, height: actualHeight }
        if (sizeDef.name) output.name = sizeDef.name
        outputs.push(output)
        this.stats.variants++
  ...
  watch() {
    ...
    // chokidar matches ignored patterns against the absolute paths it emits,
    // so patterns must be absolute. This only trims event noise — the
    // authoritative filter is isDiscovered in the drain loop, the same
    // predicate a build run uses.
    const ignored = this.config.exclude.map(e =>
      path.resolve(this.config.in, e).split(path.sep).join('/'))
    const relOut = path.relative(watchPath, path.resolve(this.config.out))
    if (relOut !== '' && !relOut.startsWith('..') && !path.isAbsolute(relOut)) {
      ignored.push(path.resolve(this.config.out).split(path.sep).join('/') + '/**')
    }
  ...
  async _processSvgPipeline(svgFiles, { force, dryRun }) {
    for (const svgFile of svgFiles) {
      ...
      try {
        ...
        this._error({ text: 'Cannot read file:', link: relativePath })
        continue
      }
      ...
      try {
        await this._processSvgFile(svgFile, stat)
      } catch (err) {
        this._error({ text: err.message + ':', link: relativePath })
      }
    }
  }

  // The one path an SVG takes: minify, run svg-enabled preprocessors, record
  // the cache entry. Shared by the build pipeline and watch mode so an edit
  // under watch regenerates the same variants a build would. Throws when the
  // minify fails; the caller counts and logs it.
  async _processSvgFile(svgFile, stat) {
    const relativePath = path.relative(this.config.in, svgFile)
    ...
    const originalSize = result.outputSize + result.saved
    const pct = originalSize > 0 ? Math.round((result.saved / originalSize) * 100) : 0
    this._info({
      text: 'Minified:',
      link: relativePath,
      size: `${formatBytes(result.outputSize)} (${pct > 0 ? '-' + pct + '%' : 'same'})`
    })
  ...
  async _processGifPipeline(gifFiles, { force, dryRun }) {
    for (const gifFile of gifFiles) {
      ...
      if ((meta.pages || 1) > 1) {
        ...
        if (this._refusesOverwrite(outPath, gifFile, relativePath)) continue
  ...
  async _drainWatchQueue() {
    ...
    while (this._watchQueue.length > 0) {
      ...
      try {
        if (type === 'unlink') {
          this._watchRemoveOutputs(filePath)
        } else if (this.cache.foreignOutputs(this.config.in).has(path.resolve(filePath))) {
          // A file this tool wrote — processing it would re-ingest an output
          continue
        } else if (!(await isDiscovered(this.config, filePath))) {
          // Not a file a build run would pick up — include/exclude apply
          // to watch events exactly as they do to discovery
          continue
  ...
  async _watchProcessGif(filePath, force) {
    ...
    if ((meta.pages || 1) > 1) {
      await this._watchCopyGif(filePath, meta)
    } else {
      await this.processImage(filePath, force)
      this.cache.save()
    }
  ...
  async _watchCopyGif(filePath, meta) {
    const relativePath = path.relative(this.config.in, filePath)
    let stat
    try {
    ...
    const outPath = path.join(this.config.out, relativePath)
    if (this._refusesOverwrite(outPath, filePath, relativePath)) return
    fs.mkdirSync(path.dirname(outPath), { recursive: true })
  ...
  async _watchProcessSvg(filePath) {
    ...
    try {
      ...
      this._error({ text: 'Cannot read file:', link: path.relative(this.config.in, filePath) })
      return
    }

    // _processSvgFile throws on failure; the drain loop's catch counts and logs it
    await this._processSvgFile(filePath, stat)
    this.cache.save()
  }
}
