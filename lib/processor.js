...
export default class ImageProcessor {
    ...
    for (const preprocessor of this.config.preprocessors) {
      try {
        if (preprocessor.resizeFirst === true) {
          // Per-variant: resize each variant first, then preprocess
          await this._processVariantsResizeFirst(inputPath, job, preprocessor, outputs, startTime)
        } else if (preprocessor.resizeFirst) {
          // Object: resize to base size once, preprocess once, then generate variants
          const baseBuffer = await this._resizeToBase(inputPath, preprocessor.resizeFirst, job)
          const result = await applyOperations(baseBuffer, preprocessor.operations, this._configDir)
   ...
   * Used when resizeFirst is an object like { width: 800 }.
   * Returns a Buffer at the target dimensions.
   */
  async _resizeToBase(input, resizeDef, job) {
    const opts = {}
    if (resizeDef.width) opts.width = resizeDef.width
    if (resizeDef.height) opts.height = resizeDef.height

    // Don't upscale: clamp to source dimensions
    if (opts.width && opts.width > job.sourceWidth) opts.width = job.sourceWidth
    if (opts.height && opts.height > job.sourceHeight) opts.height = job.sourceHeight

    if (resizeDef.crop) {
      opts.fit = 'cover'
      opts.position = Array.isArray(resizeDef.crop) ? resizeDef.crop.join(' ') : (resizeDef.crop === true ? 'centre' : resizeDef.crop)
    } else {
      opts.fit = 'inside'
    ...
    let pipeline
    if (typeof input === 'string') {
      pipeline = sharp(input).rotate().resize(opts)
    } else {
      pipeline = sharp(input).resize(opts)
    }
  ...
  /**
   * Resize first, then apply preprocessor operations per variant.
   * Used when preprocessor.resizeFirst is true — the operations run on each
   * already-resized variant instead of on the full-size source.
   */
  async _processVariantsResizeFirst(input, job, preprocessor, outputs, startTime) {
    const relativePath = job.relativePath
    ...
    } else if (preprocessor.skipOriginal != null) {
      sizes = buildEffectiveSizes(this.config.sizes, skipOrig)
    } else {
      sizes = job.effectiveSizes
    }
    ...
    const namePrefix = `${job.parsed.name}-${preprocessor.name}`
    const eligibleSizes = filterByUpscale(sizes, job.sourceWidth, job.sourceHeight)
    ...
    for (const sizeDef of eligibleSizes) {
      ...
      // Step 2: Apply preprocessor operations on the resized buffer
      const ppResult = await applyOperations(resizedBuffer.data, preprocessor.operations, this._configDir)
      const actualWidth = ppResult.info.width
      const actualHeight = ppResult.info.height

      // Write sidecars only once (for the first / largest variant)
      if (ppResult.sidecars && ppResult.sidecars.length > 0 && sizeDef === eligibleSizes[0]) {
      ...
      const { formats: targetFormats, preEncoded } = await resolveTargetFormats(
        job.outputExt, job.transparent, ppResult.data,
        format, quality
      )
      ...
      for (const targetExt of targetFormats) {
        ...
        try {
          buffer = preEncoded.get(targetExt) || await convertFormat(
            sharp(ppResult.data), targetExt, quality
          )
        } catch (err) {
          log({ tag: 'error', text: `Format conversion failed (${targetExt}): ${err.message}`, link: relativePath })
          continue
        ...
        log({
          tag: 'image',
          text: `Compiled [${preprocessor.name}]:`,
          link: relPath,
          size: formatBytes(buffer.length),
          time: formatTime(Date.now() - startTime)
        })
