...
export function validatePreprocessor(preprocessor) {
  ...
  return {
    ...
    resizeFirst: preprocessor.resizeFirst === true ? true : (preprocessor.resizeFirst || false)
  }
}
...
export async function applyOperations(input, operations, configDir) {
  ...
  let pipeline = (!isBuffer && !hasExplicitRotate) ? sharp(input).rotate() : sharp(input)
  const sidecars = []
