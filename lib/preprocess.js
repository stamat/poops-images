...
// Cache for loaded custom handler modules, one entry per handler path — a new
// mtime replaces the old entry, so the map cannot grow with edits under watch.
// The ESM module registry still retains every imported version for the process
// lifetime; that is the price of cache-busting import(), and it is bounded by
// how often a human edits a handler between restarts.
...
async function loadHandler(name, configDir) {
  ...
  // import() caches by URL for the process lifetime, so the import URL is
  // keyed by mtime — an edited handler loads fresh on the next run
  const mtime = fs.statSync(resolved).mtimeMs
  const cached = handlerCache.get(resolved)
  ...
  handlerCache.set(resolved, { mtime, fn })
  return fn
}
...
function validateResizeFirst(value, name) {
  ...
  if (typeof value === 'object' && !Array.isArray(value)) {
    // 0 counts as unset, so an omitted dimension and one already normalized to
    // 0 by a previous pass both have to pass — validating a validated config
    // must not start rejecting it
...
export function validatePreprocessor(preprocessor) {
  ...
  // Same validation as top-level sizes — a bad crop anchor surfaces here as
  // one config error, not as a per-image sharp failure at process time
  let sizes = null
  if (preprocessor.sizes != null) {
    if (!Array.isArray(preprocessor.sizes)) {
      throw new Error(`Preprocessor "${preprocessor.name}": "sizes" must be an array`)
    }
    sizes = preprocessor.sizes.map(validateSize)
  }
